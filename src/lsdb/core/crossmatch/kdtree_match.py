...
class KdTreeCrossmatch(AbstractCrossmatchAlgorithm):
    ...
    def perform_crossmatch(
        ...
        chord_distances, left_idx, right_idx = _find_crossmatch_indices(
            left_xyz, right_xyz, how=self.how, n_neighbors=n_neighbors, max_distance=max_d_chord
        )
    ...
    def _get_extra_columns_df(self, chord_distances: npt.NDArray[np.float64]) -> pd.DataFrame:
        chord_distances = chord_distances[~np.isinf(chord_distances)]
        arc_distances = _get_arc_separation(chord_distances)
        return pd.DataFrame({"_dist_arcsec": pd.Series(arc_distances, dtype=pd.ArrowDtype(pa.float64()))})
