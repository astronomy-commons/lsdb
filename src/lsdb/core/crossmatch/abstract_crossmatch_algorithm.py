...
class AbstractCrossmatchAlgorithm(ABC):
    ...
    def validate(self, left: Catalog, right: Catalog):
        ...
        if left.hc_structure.catalog_info.ra_column not in column_names:
            raise ValueError(
                f"left table '{left.name}' must have column '{left.hc_structure.catalog_info.ra_column}'"
            )
        if left.hc_structure.catalog_info.dec_column not in column_names:
            raise ValueError(
                f"left table '{left.name}' must have column '{left.hc_structure.catalog_info.dec_column}'"
            )

        column_names = right.columns
        if right.hc_structure.catalog_info.ra_column not in column_names:
            raise ValueError(
                f"right table '{right.name}' must have column '{right.hc_structure.catalog_info.ra_column}'"
            )
        if right.hc_structure.catalog_info.dec_column not in column_names:
            raise ValueError(
                f"right table '{right.name}' must have column '{right.hc_structure.catalog_info.dec_column}'"
            )
