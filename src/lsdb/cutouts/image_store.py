...
from nested_pandas import ImageStore as _NestedImageStore


class ImageStore(_NestedImageStore):
    """Resolves image ids to pixel arrays (and optionally WCS objects).

    Extends the nested-pandas store interface (which cutout columns resolve
    pixels through) with WCS lookup and edge clamping.
    """
