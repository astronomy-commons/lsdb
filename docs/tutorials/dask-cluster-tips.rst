...
You will see these tasks as ``to_pyarrow_string`` if you look at the dask task graphs generated by LSDB.

LSDB and hats by default use pyarrow columns for all data types, so these conversion tasks take a minimal
amount of time to check the column types. However, if you switch to using different dtype backends when
...
To turn off these tasks, you can change the default Dask configuration for string conversion with the
following code:
