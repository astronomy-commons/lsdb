...
def test_match_basic():
    ...
    by_position = {round(objects["ra"].iloc[i], 2): cutouts[i] for i in range(len(objects))}
    assert by_position[179.9].image_id == "v1"
    assert by_position[180.4].image_id == "v2"
    assert by_position[180.15].image_id in ("v1", "v2")  # overlap: first fit
    assert by_position[190.0] is pd.NA
...
def test_match_falls_through_to_second_image_at_edge():
    ...
    assert small[0].image_id == "v1"
    assert big[0].image_id == "v2"
