...
def test_kdtree_right_columns(kdtree_crossmatch):
    ...
    kdtree_crossmatch.right = original_df.drop(columns=["ra"])
    with pytest.raises(ValueError, match="right table must have column ra"):
        kdtree_crossmatch.validate()

    kdtree_crossmatch.right = original_df.drop(columns=["dec"])
    with pytest.raises(ValueError, match="right table must have column dec"):
        kdtree_crossmatch.validate()
