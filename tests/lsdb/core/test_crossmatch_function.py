...
def test_dataframe_or_catalog_crossmatch(
    left, right, small_sky_catalog, small_sky_xmatch_catalog, xmatch_correct
):
...
def test_invalid_type_crossmatch(left, right, small_sky_catalog, small_sky_xmatch_catalog):
    ...
    if right == "invalid":
        with pytest.raises(TypeError, match="Argument must be"):
            lsdb.crossmatch(small_sky_catalog, np.array([1, 2, 3]))
        return


def test_invalid_margin_args_crossmatch(small_sky_catalog, small_sky_xmatch_catalog):
...
def test_ra_dec_columns_crossmatch(small_sky_catalog, small_sky_xmatch_catalog, xmatch_correct):
    """Raise an error if an impossible margin argument combination is given."""
    ...
    result = lsdb.crossmatch(
        ...
        right_args={
            ...
            "ra_column": "abnormal_rightasc_col_name",
        },
    ).compute()
    assert isinstance(result, npd.NestedFrame)
    assert len(result) == len(xmatch_correct)
    ...
    right_dataframe_abnormal_cols = right_dataframe.rename(
        columns={"ra": "abnormal_rightasc_col_name", "dec": "abnormal_de_col_name"}
    )
    ...
    result = lsdb.crossmatch(
        ...
        radius_arcsec=0.01 * 3600,
        ra_column="abnormal_rightasc_col_name",
        dec_column="abnormal_de_col_name",
        right_args={
            "margin_threshold": 100,
            "ra_column": "abnormal_rightasc_col_name",
        },
