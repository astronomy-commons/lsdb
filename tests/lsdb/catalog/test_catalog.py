...
def test_empty_dataframe():
    empty_df = pd.DataFrame({"ra": [], "dec": [], "id": []})
    empty_cat = lsdb.from_dataframe(empty_df)
    html = empty_cat.__repr_html__()
    assert "0 rows" in html or "Unknown" in html
...
def test_repr_html_is_lazy():
    ...
    assert elapsed < 1.0, f"__repr_html__ took {elapsed:.2f}s, should be < 1s"
    assert peak_mem < 50 * 1024 * 1024, f"Used {peak_mem / 1024 / 1024:.2f} MB, should be < 50 MB"
